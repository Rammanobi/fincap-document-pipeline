...
def read_page_with_vision(image_path: str, page_number: int) -> str:
    ...
    try:
        ...
        if not content or len(content.strip()) < 20:
            print(f"Warning: Vision returned little/no text for page {page_number}. Retrying once...")
            content = do_call()
            if not content or len(content.strip()) < 20:
                print(f"Warning: Vision still returned little/no text for page {page_number} after retry.")
                return f"[VISION FAILED - PAGE {page_number} - MANUAL REVIEW NEEDED]"
